...
class SSC_API_Attribute {
	API_Attribute	attr;
public:
	SSC_API_Attribute (API_AttrTypeID typeID, API_AttributeIndex index);

	friend ostream& operator<< (ostream &outs, const SSC_API_Attribute &v);
	ostream& print (ostream &outs) const;
};

SSC_API_Attribute::SSC_API_Attribute (API_AttrTypeID typeID, API_AttributeIndex index)
{
	attr			   = {};
...
ostream& SSC_API_Attribute::print (ostream &outs) const
{
	if (attr.header.typeID == API_FilltypeID &&
		...
		outs << " = " << attr.header.name << " (#" << attr.header.index.ToUniString ().ToCStr ().Get () << ", " << (attr.filltype.subType == APIFill_Solid ? "solid" : "empty") << ")" << endl;
	} else
		outs << " = " << attr.header.name << " (#" << attr.header.index.ToUniString ().ToCStr ().Get () << ")" << endl;
...
{
	outs << "\tpen = (" << pen.penIndex << ", " << pen.colorOverridePenIndex << ')' << endl;
...
ostream& SSC_API_PrimHatchBorder::print (ostream &outs) const
{
	...
	outs << "\tpen = (" << fillPen.penIndex << ", "  << fillPen.colorOverridePenIndex << "), bgPen = " << fillbkgPen << endl;
...
ostream& SSC_API_PrimText::print (ostream &outs) const
{
	...
	API_FontType apiFont = {};
	apiFont.head.index = font;
	apiFont.head.flags |= 0x8000;
	ACAPI_Font_GetFont (apiFont);
	outs << " = " << apiFont.head.name << " (#" << apiFont.head.index << ")" << endl;
					...
					const void* par1, const void* par2, const void* /*par3*/)
{
	...
	switch (primElem->header.typeID) {
		...
		case API_PrimTextID:
			...
			if (par2 != nullptr && *(reinterpret_cast<const unsigned short*> (par2)) != 0) {
				GS::UniString ustr (reinterpret_cast<const GS::UniChar::Layout*> (par2));
				vlist << "\ttext = \"" << ustr.ToCStr ().Get () << "\"";
			}
			break;
		...
		case API_PrimCtrl_ElementRefID:
			...
			vlist << "\ttypeID = " << (int) ((API_PrimElemRef *) par1)->type.typeID << ", guid = " << APIGuidToString (((API_PrimElemRef *) par1)->guid).ToCStr ().Get () << endl;
			break;
		...
		case API_PrimCtrl_TextBegID:
			...
			vlist << SSC_API_PrimText (primElem->text);
			if (par1 != nullptr && strlen ((const char *) par1) > 0)
		...
		case API_PrimCtrl_BackgroundFillBegID:
			vlist << "+ API_PrimCtrl_BackgroundFillBegID" << endl;
			break;

		case API_PrimCtrl_BackgroundFillEndID:
			vlist << "- API_PrimCtrl_BackgroundFillEndID" << endl;
			break;
	}
