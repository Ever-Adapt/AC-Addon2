...
void		Do_Modify_Opening (void)
{
	...
	if (!ClickAnElem ("Click an element containing Openings", API_ZombieElemID, nullptr, &element.header.type, &element.header.guid)) {
		WriteReport_Alert ("No object was clicked");
		return;
	}
	...
	ACAPI_Grouping_GetConnectedElements (element.header.guid, API_OpeningID, &openingsGuid);
...
void	Do_BeamSegmentExtrusionMaterial_Edit (void)
{
	API_Element			element = {};
	API_ElementMemo		memo = {};
	GS::Array<API_Elem_Head> elems;

	if (!ClickAnElem ("Click a beam to override its segments extrusion material", API_BeamID, nullptr, &element.header.type, &element.header.guid)) {
		WriteReport_Alert ("No beam was clicked");
		return;
	}
	...
	API_OverriddenAttribute newExtrusionMaterial;
	newExtrusionMaterial.value = ACAPI_CreateAttributeIndex (11);
	newExtrusionMaterial.hasValue = true;

	GSSize nSegments = BMGetPtrSize (reinterpret_cast<GSPtr>(memo.beamSegments)) / sizeof (API_BeamSegmentType);
	for (GSSize idx = 0; idx < nSegments; ++idx) {
		err = ACAPI_Element_ChangeBeamSegment_ExtrusionMaterial (&memo.beamSegments[idx], newExtrusionMaterial, true, true);
	}

	if (err != NoError)
		WriteReport_Alert ("Error: ACAPI_Element_ChangeBeamSegment_ExtrusionMaterial failed!");
